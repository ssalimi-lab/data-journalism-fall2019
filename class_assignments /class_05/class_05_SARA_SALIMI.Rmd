...
```{r}
#loading in the four sets of data we'll be working with 
...
#Buyer annual contains all pills sent to all counties for all the years. The totals summarize it much better and therefore contain less entries. Buyer address are higher because it goes into individual pharmacy addresses for each county, but buyer totals sum that up and don't include separate pharmacies. It just has the county and state. 
```
...
pills_population_left <- buyer_annual_by_year %>%
  left_join(buyer_addresses, by="buyer_dea_no")

#Retail Pharmacy in Charleston County, SC. It may be because there are multiple Retail Pharmacies in different counties and in different states. 
```
...
# 	MATHUR, RAKESH K MD	; total pills: 337,450 which is 124,650 pills more than the next highest doctor. His license was suspended and his office closed in Feb. 2012. 
...
#WV had the highest rate of total pills per person 
...
# no addresses found, zero records returned. 
...
```{r}
pills_population_check <- buyer_addresses %>%
  inner_join(buyer_totals, by="buyer_dea_no")

#doing an inner join to check the number of matching records. 
