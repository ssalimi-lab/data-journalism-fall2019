...
```{r}
buyer_details_mont <- buyer_details(county="Montgomery", state="MD", key = key) 

county_population_mont <- county_population(county="Montgomery", state="MD", key = key)

summarized_county_annual_mont <- summarized_county_annual(county="Montgomery", state = "MD", key = key)
...
```{r}
#initial ideas: Is there a relationship betweeen opioid rates and poverty/income levels nationally and in specific cities or counties? Were opioid death rates worse in less-well-off areas of the country? Places to start: American Community Survey (sample done every year on a rolling basis, like a poll). Pacakge in R - tidycensus. 

#Cleaning data 
buyer_details_mont <- clean_names(buyer_details_mont)
county_population_mont <- clean_names(county_population_mont)
summarized_county_annual_mont <- clean_names(summarized_county_annual_mont)

```

```{r}
#Doing some filtering to see data for individual cities
buyer_details_mont %>%
  filter(buyer_city == "GAITHERSBURG")

buyer_details_mont %>%
  filter(buyer_city == "POTOMAC")

buyer_details_mont %>%
  filter(buyer_city == "BETHESDA")

buyer_details_mont %>%
  filter(buyer_city == "ROCKVILLE")
