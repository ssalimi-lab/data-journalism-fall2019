...
```{r}
  ...
  labs(x="Year", y="Total pills", title="In Logan County, opioids remained relatively steady between 2006 and 2012, with a sudden decrease in 2010 which climbed back up in the next year.", subtitle = "Total pills shipped to Logan County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)

  ggsave("logan_pills.png", plot=last_plot())
  
  #Logan County seems to have experienced relatively steady increases and decreases in opioids. The numbers increased from six million pills in 2006 to around 6.5 million in 2009. In 2010, there was a  drop back to six million pills, which then increased the following year. The county did not experience dramatic increases or decreases in opioids throughout the 2006-2012 period. 
```
...
wv_pills_per_year <- arcos_county_pills_per_year %>%
  filter(buyer_state == "WV", (buyer_county %in% c("LOGAN", "MINGO", "CABELL","KANAWHA", "RALEIGH", "WOOD"))) %>%
...
```{r}
  ...
  labs(x="Year", y="Total pills", title="Steady rise in opioids in six West Virginia counties", subtitle = "Total pills per year shipped to Logan, Mingo, Cabell, Kanawha, Raleigh and Wood counties", caption = "Source: DEA ARCOS database, via Washington Post", fill="County") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma) +
...
# The individual bar charts reveal that opioids have been steadily increasing in six West Virginia counties. While some experienced sudden increases, the general trend shows that the number of pills increased relatively steadily between 2006 to 2012. 
```
...
#Had a hard time figuring out how the join would work in this case.
```
...
```{r}
montgomery_county_pills_per_year <- arcos_county_pills_per_year %>%
  filter(buyer_state == "MD", buyer_county == "MONTGOMERY") %>%
  select(year, dosage_unit)
```

```{r}
ggplot(montgomery_county_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="orange") +
  labs(x="Year", y="Total pills", title="In Montgomery County, opioids steadily increased from 2006 to 2010, and very minimally decreased until 2012.", subtitle = "Total pills shipped to Montgomery County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)

  ggsave("montgomery_pills.png", plot=last_plot())
