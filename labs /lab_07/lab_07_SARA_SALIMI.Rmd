...
```{r}
# The not_in_pills table contains county fips, which could be used as an identifier, but the not_in_population table doens't have that. This could make it hard to trace where exactly these pills went. 
```
...
```{r}
# Making sure it joins properly so that it's not missed in data analysis 
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per person',title="West Virginia has the highest number of pills per person", subtitle = "2009 pills per person, U.S. Census", caption = "Source: U.S. Census ACS") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person per state", subtitle = "Between 2006 to 2012, the number of pills per person increased for each state.", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
geo_pills_population_county <- county_geodata_shifted %>%
  inner_join (pills_population_county, by=c("GEOID" = "countyfips"))

mapview(geo_pills_population_county, zcol = "pills_per_person", legend = TRUE)
...
geo_pills_population_MD <- geo_pills_population_county %>%
